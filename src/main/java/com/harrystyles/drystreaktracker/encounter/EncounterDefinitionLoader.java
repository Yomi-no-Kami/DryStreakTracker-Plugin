...
public class EncounterDefinitionLoader
{
    ...
    private static final String LOOT_RECEIVED_RESOURCE =
            "/loot-received-encounters.json";
            ...
            EncounterRegistry registry)
    {
        if (registry == null)
        {
            throw new IllegalArgumentException(
                    "EncounterRegistry cannot be null"
            );
        }
         ...
         */
        EncounterDefinition[] npcDefinitions =
                loadResource(
                        NPC_RESOURCE,
         ...
         */
        EncounterDefinition[] lootReceivedDefinitions =
                loadResource(
                        LOOT_RECEIVED_RESOURCE,
                        EncounterLootType.LOOT_RECEIVED
                );

        for (EncounterDefinition definition
                : lootReceivedDefinitions)
        {
            validate(
                    definition
            );
        ...
        log.info(
                "Encounter registry initialized with {} encounters",
                registry.size()
        );
    }
     ...
     */
    private EncounterDefinition[] loadResource(
            String resource,
            EncounterLootType lootType)
    {
        ...
        if (inputStream == null)
        {
            ...
            {
                log.warn(
                        "Optional LootReceived encounter resource not found: {}",
                        resource
                );
        ...
        )
        {
                    ...
                    : definitions)
            {
                 ...
                 */
                definition.setLootType(
                        lootType
                );
            }

            log.info(
                    "Loaded {} encounters from {} as {}",
                    definitions.length,
                    resource,
                    lootType
            );
            ...
            EncounterDefinition definition)
    {
        ...
        {
            throw new IllegalStateException(
                    "Encounter definition cannot be null"
            );
        }
        ...
        {
            throw new IllegalStateException(
                    "Encounter definition has no encounterId"
            );
        }
        ...
        if (definition.getLootType() == null)
        {
            throw new IllegalStateException(
                    "Encounter "
                            + definition.getEncounterId()
                            + " has no internally assigned loot type"
            );
        }
                ...
                == EncounterLootType.GROUND_LOOT)
        {
            ...
            {
                log.warn(
                        "Encounter {} is in {} but contains lootSourceNames. "
                                + "lootSourceNames will not be used.",
                        definition.getEncounterId(),
                        NPC_RESOURCE
                );
            }
                ...
                == EncounterLootType.LOOT_RECEIVED)
        {
            ...
            {
                throw new IllegalStateException(
                        "LootReceived encounter "
                                + definition.getEncounterId()
                                + " from "
                                + LOOT_RECEIVED_RESOURCE
                                + " must have at least one lootSourceName"
                );
            }
        }
    }
